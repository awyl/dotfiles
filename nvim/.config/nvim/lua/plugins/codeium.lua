...
return {
  ...
  dependencies = {
    {
      ...
      opts = function(_, opts)
        ...
        maps.i["<C-g>"] = { function() require("neocodeium").accept() end }
  ...
  opts = {
    ...
    filter = function(bufnr)
      if
        -- don't enable for .env files
        vim.endswith(vim.api.nvim_buf_get_name(bufnr), ".env")
        -- don't enable for secrets folders
        or (vim.fn.expand "%:p" ~= "" and vim.fn.expand("%:p"):find "/secrets/" ~= nil)
      then
        return false
      end

      -- enable for anything else
      return true
    end,
  },
}
